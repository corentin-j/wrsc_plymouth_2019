...
def kalman_predict(xup,Gup,u,Galpha,A):
    G1 = np.dot(np.dot(A,Gup),A.T) + Galpha
    x1 = np.dot(A,xup) + u  
    return(x1,G1)    
...
def use_kalman(xhat, Gx, y_angle, gyro):
	dt = vect_temps[2]				
	u = np.zeros((2,1))
	theta = np.arctan2(my,mx)
	A = np.array([[1,-dt*gz],[dt*gz,1]])
	C = np.array([[1,0],[0,1]])
	y = np.array([[np.cos(y_angle)],[np.sin(y_angle)]])
	[xhat, Gx] = kalman_predict(xhat,Gx,u,Galpha,A)
	[xhat, Gx] = kalman_correc(xhat,Gx,y,Gbeta,C)

	return (xhat, Gx)

def kalman_to_deg(xhat):
	return np.arctan2(xhat[1,0],xhat[0,0])*180/np.pi

def kalman_to_rad(xhat):
	return np.arctan2(xhat[1,0],xhat[0,0])
...
def F(x,u):
	dt = vect_temps[2]
	mat = np.array([[1,0,0],[0,1,0],[0,0,1]])#np.array([[1,-dt*u,0],[dt*u,1,0],[0,0,1]])
	return mat
...
if __name__ == '__main__':
	...
	try:
		...
		rospy.Subscriber("ImuNoFiltre", Imu, sub_imu_nf)
		rospy.Subscriber("MagNoFiltre", MagneticField, sub_mag_nf)
